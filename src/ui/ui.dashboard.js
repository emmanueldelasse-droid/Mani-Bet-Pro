...
function _createMatchCard(match) {
  ...
  function _fmtML(am)  { if (am == null) return null; return am > 0 ? `+${am}` : String(am); }
  ...
  const oddsSource = pinnacle
    ? (pinnacle.key === 'winamax' ? 'Winamax' : 'Pinnacle')
    : (espnOdds.home_ml != null ? 'ESPN' : null);

  const espnSpread = espnOdds.spread;
  const spread     = espnSpread != null ? (espnSpread > 0 ? `+${espnSpread}` : String(espnSpread))
  ...
  const centerContent = showScore
    ? `<div style="font-family:var(--font-mono);font-size:18px;font-weight:800;line-height:1;text-align:center">
         ${homeScore} <span style="font-size:12px;opacity:0.4">–</span> ${awayScore}
       </div>`
    : `<div style="font-family:var(--font-mono);font-size:11px;font-weight:700;color:var(--color-text-muted);letter-spacing:0.10em">VS</div>
       ${ou    ? `<div style="font-size:9px;color:var(--color-text-muted);margin-top:3px">O/U ${ou}</div>` : ''}
       ${spread ? `<div style="font-size:9px;color:var(--color-text-muted)">Hcp ${spread}</div>` : ''}`;
  ...
  card.innerHTML = `
    ...
    <!-- ÉQUIPES face à face -->
    <div style="display:grid;grid-template-columns:1fr 56px 1fr;align-items:start;gap:0;margin:2px 0 0">

      <!-- DOM -->
      <div style="display:flex;flex-direction:column;gap:1px">
        <span style="font-family:var(--font-mono);font-size:22px;font-weight:800;letter-spacing:-0.02em;line-height:1;color:var(--color-text-primary)">${match.home_team?.abbreviation ?? '—'}</span>
        <span style="font-size:11px;color:var(--color-text-secondary);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:130px">${match.home_team?.name ?? '—'}</span>
        <span style="font-family:var(--font-mono);font-size:10px;color:var(--color-text-muted);margin-top:1px">${homeRecord}</span>
        <!-- cote ML -->
        <span id="odds-home-${match.id}" style="font-family:var(--font-mono);font-size:17px;font-weight:700;color:var(--color-text-primary);margin-top:7px;line-height:1">${homeML ?? '—'}</span>
        ${oddsSource ? `<span style="font-size:9px;color:var(--color-text-muted);margin-top:1px">${oddsSource}</span>` : ''}
        <!-- prob analyse (injectée par _updateMatchCard) -->
        <span id="motor-home-${match.id}" style="display:none;font-size:10px;font-weight:600;margin-top:2px"></span>
        <span id="market-home-${match.id}" style="display:none"></span>
      </div>

      <!-- CENTRE -->
      <div style="display:flex;flex-direction:column;align-items:center;justify-content:flex-start;padding-top:4px;gap:2px">
        ${centerContent}
      </div>

      <!-- EXT -->
      <div style="display:flex;flex-direction:column;gap:1px;align-items:flex-end;text-align:right">
        <span style="font-family:var(--font-mono);font-size:22px;font-weight:800;letter-spacing:-0.02em;line-height:1;color:var(--color-text-primary)">${match.away_team?.abbreviation ?? '—'}</span>
        <span style="font-size:11px;color:var(--color-text-secondary);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:130px">${match.away_team?.name ?? '—'}</span>
        <span style="font-family:var(--font-mono);font-size:10px;color:var(--color-text-muted);margin-top:1px">${awayRecord}</span>
        <!-- cote ML -->
        <span id="odds-away-${match.id}" style="font-family:var(--font-mono);font-size:17px;font-weight:700;color:var(--color-text-primary);margin-top:7px;line-height:1">${awayML ?? '—'}</span>
        ${oddsSource ? `<span style="font-size:9px;color:var(--color-text-muted);margin-top:1px">${oddsSource}</span>` : ''}
        <span id="motor-away-${match.id}" style="display:none;font-size:10px;font-weight:600;margin-top:2px"></span>
        <span id="market-away-${match.id}" style="display:none"></span>
      </div>
    </div>

    <!-- BARRE PROBA (injectée par _updateMatchCard) -->
    <div id="proba-bar-${match.id}" style="display:none;margin-top:2px">
      <div style="height:3px;border-radius:2px;overflow:hidden;background:var(--color-border-default)">
        <div id="proba-fill-${match.id}" style="height:100%;border-radius:2px;background:linear-gradient(90deg,var(--color-signal),var(--color-signal-light));width:50%;transition:width 0.7s cubic-bezier(0.4,0,0.2,1)"></div>
      </div>
    ...
    <div id="edge-${match.id}" style="display:none"><span id="edge-val-${match.id}"></span><span id="quality-val-${match.id}"></span></div>

    <!-- NIVEAU (injecté par _updateMatchCard) -->
    <div id="level-${match.id}" style="display:none"></div>

    <!-- MEILLEURE REC (injectée par _updateMatchCard) -->
    <div id="best-rec-${match.id}" style="display:none"></div>

    <!-- RECS supplémentaires -->
    ...
    <div style="display:flex;align-items:center;justify-content:space-between;gap:8px;margin-top:2px">
      ...
      <button class="btn btn--ghost match-card__cta" data-match-id="${match.id}" data-analysis-id=""
        style="margin-top:0;padding:5px 12px;font-size:11px;width:auto;flex-shrink:0">
        → Voir analyse
      </button>
    </div>
  `;
